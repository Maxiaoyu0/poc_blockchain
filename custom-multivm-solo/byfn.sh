...
function askProceed() {
  echo $SHOULD_ASK_PROCEED
  if [ $SHOULD_ASK_PROCEED -eq true ]; then
...
function upgradeNetwork() {
  if [[ "$IMAGETAG" == *"1.4"* ]] || [[ $IMAGETAG == "latest" ]]; then
    docker inspect -f '{{.Config.Volumes}}' orderer.ticketChain.sfeir.lu | grep -q '/var/hyperledger/production/orderer'
    if [ $? -ne 0 ]; then
    ...
    docker-compose $COMPOSE_FILES stop orderer.ticketChain.sfeir.lu
    docker cp -a orderer.ticketChain.sfeir.lu:/var/hyperledger/production/orderer $LEDGERS_BACKUP/orderer.ticketChain.sfeir.lu
    docker-compose $COMPOSE_FILES up -d --no-deps orderer.ticketChain.sfeir.lu

    for PEER in peer0.sfeir.ticketChain.sfeir.lu peer1.sfeir.ticketChain.sfeir.lu peer0.ticketMaster.ticketChain.sfeir.lu peer1.ticketMaster.ticketChain.sfeir.lu peer0.bankRoute.ticketChain.sfeir.lu peer1.bankRoute.ticketChain.sfeir.lu; do
...
function networkDown() {
  # stop org3 containers also in addition to sfeir, ticketMaster and bankRoute, in case we were running sample to add org3
...
function replacePrivateKey() {
  ...
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/sfeir.ticketChain.sfeir.lu/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA1_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
  cd crypto-config/peerOrganizations/ticketMaster.ticketChain.sfeir.lu/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA2_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
  cd crypto-config/peerOrganizations/bankRoute.ticketChain.sfeir.lu/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
...
# and two Peer Orgs (``Sfeir`` & ``TicketMaster``) each managing and maintaining two peer nodes.
...
# (``peer0.sfeir.ticketChain.sfeir.lu`` & ``peer0.ticketMaster.ticketChain.sfeir.lu`` & ``peer0.bankRoute.ticketChain.sfeir.lu``).  
...
function generateChannelArtifacts() {
  ...
  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for SfeirMSP   ##########"
  echo "#################################################################"
  set -x
  configtxgen -profile ThreeOrgsChannel -outputAnchorPeersUpdate ./channel-artifacts/SfeirMSPanchors.tx -channelID $CHANNEL_NAME -asOrg SfeirMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for SfeirMSP..."
    exit 1
  fi

  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for TicketMasterMSP   ##########"
  echo "#################################################################"
  set -x
  configtxgen -profile ThreeOrgsChannel -outputAnchorPeersUpdate \
    ./channel-artifacts/TicketMasterMSPanchors.tx -channelID $CHANNEL_NAME -asOrg TicketMasterMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for TicketMasterMSP..."
    exit 1
  fi
  echo
  ...
  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for BankRouteMSP   ##########"
  echo "#################################################################"
  set -x
  configtxgen -profile ThreeOrgsChannel -outputAnchorPeersUpdate \
    ./channel-artifacts/BankRouteMSPanchors.tx -channelID $CHANNEL_NAME -asOrg BankRouteMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for BankRouteMSP..."
    exit 1
  fi
  echo
}
...
CONSENSUS_TYPE="solo"
# default skip askProceed
SHOULD_ASK_PROCEED=true
...
while getopts "h?c:t:d:f:s:l:i:o:vy" opt; do
  case "$opt" in
  h | \?)
    printHelp
    exit 0
